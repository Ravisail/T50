    ...
    def run_backtest(self, data_dict: Dict[str, pd.DataFrame]) -> Tuple[List[CompletedTrade], List[Tuple[datetime, float]]]:
        ...
        for current_date in sorted_dates:
            ...
            for symbol, order in list(pending_average_orders.items()):
                if (symbol in prepared_data and current_date in prepared_data[symbol].index
                        and symbol in portfolio.positions and not self.config.stop_loss_mode):
                    row = prepared_data[symbol].loc[current_date]
                    execute_price = float(row['open'])
...
def calculate_xirr(cashflows: List[Tuple[Any, float]]) -> float:
    ...
    if f_hi == 0:
        return hi * 100
    if f_lo * f_hi > 0:
        # Cash flows don't bracket a root in this range (e.g. all-negative /
        # all-positive series) - can't solve reliably, report 0 rather than
        # guessing.
        return 0.0
...
def get_portfolio_conn() -> sqlite3.Connection:
    conn = sqlite3.connect(str(PORTFOLIO_DB_PATH), check_same_thread=False)
...
def pf_add_average_entry(position_id: int, date_val, price: float, qty: float,
    ...
    pf_update_position(position_id, entries_json=json.dumps(entries))
...
def pf_position_metrics(pos: Dict[str, Any]) -> Dict[str, Any]:
    ...
    holding_days = 0
    try:
        bd = datetime.fromisoformat(str(buy_date)[:10])
    ...
    for row in rows:
        ...
        for c in columns:
            ...
            if kind == 'badge' and c.get('badge_fn'):
                ...
                cells.append(f'<td><span class="badge {css_class}">{label}</span></td>')
            elif kind == 'pnl':
                if raw is None:
                    cells.append('<td>—</td>')
                else:
                    try:
                        ...
                        numeric = 0.0
                    cells.append(f'<td><span class="{_pnl_class(numeric)}">{fmt(raw)}</span></td>')
            else:
                display_val = "—" if raw is None else fmt(raw)
