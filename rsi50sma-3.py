        ...
        for current_date in sorted_dates:
            ...
            for symbol, df in prepared_data.items():
                ...
                if has_position:
                    ...
                    # Check price-based target on today's close
                    if close >= exit_target:
                        profit_pct, _ = pos.calculate_unrealized_pnl(close, self.config)
                        if pos.average_count > 1:
                            reason = (f"Averaged Exit - WAP Target ₹{exit_target:.2f} "
                                      f"({profit_pct:.2f}%)")
                        else:
                            reason = f"Profit Target ₹{exit_target:.2f} ({profit_pct:.2f}%)"
                        portfolio.close_position(symbol, current_date, close, self.config, reason)
                    else:
